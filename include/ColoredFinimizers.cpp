...
struct IColorSet {
    virtual ~IColorSet() = default;

    virtual size_t size() const = 0;
    virtual sdsl::bit_vector to_bitvector(size_t n_colors) const = 0;

    virtual void serialize(std::ostream& out) const = 0;
    virtual void load(std::istream& in, size_t n_colors) = 0;

    virtual uint8_t type_id() const = 0; // needed to deserialize
};

struct SparseColorSet : public IColorSet {
    std::vector<uint32_t> gaps;
    size_t count = 0;

    SparseColorSet() = default;
    explicit SparseColorSet(const sdsl::bit_vector& bv);

    size_t size() const override;
    sdsl::bit_vector to_bitvector(size_t n_colors) const override;

    void serialize(std::ostream& out) const override;
    void load(std::istream& in, size_t n_colors) override;

    uint8_t type_id() const override { return 0; }
};

SparseColorSet::SparseColorSet(const sdsl::bit_vector& bv) {
    uint32_t prev = 0;
...
size_t SparseColorSet::size() const { return count; }
sdsl::bit_vector SparseColorSet::to_bitvector(size_t n_colors) const {
    sdsl::bit_vector out(n_colors, 0);
    size_t pos = 0;
    for (auto g : gaps) {
        pos += g;
        out[pos] = 1;
...
void SparseColorSet::serialize(std::ostream& out) const {
    uint8_t tid = type_id();
...
void SparseColorSet::load(std::istream& in, size_t /*n_colors*/) {
...
struct BitmapColorSet : public IColorSet {
    sdsl::bit_vector bv;

    BitmapColorSet() = default;
    explicit BitmapColorSet(const sdsl::bit_vector& bv_);
    ...
    uint8_t type_id() const override { return 1; }
};

BitmapColorSet::BitmapColorSet(const sdsl::bit_vector& bv_) : bv(bv_) {}
size_t BitmapColorSet::size() const { return sdsl::util::cnt_one_bits(bv); }
sdsl::bit_vector BitmapColorSet::to_bitvector(size_t) const { return bv; }
void BitmapColorSet::serialize(std::ostream& out) const {
    uint8_t tid = type_id();
    out.write(reinterpret_cast<const char*>(&tid), sizeof(tid));
    sdsl::serialize(bv, out);
}
void BitmapColorSet::load(std::istream& in, size_t /*n_colors*/) {
    sdsl::load(bv, in);
}

struct ComplementColorSet : public IColorSet {
    std::vector<uint32_t> gaps;
    size_t count = 0;

    ComplementColorSet() = default;
    explicit ComplementColorSet(const sdsl::bit_vector& bv);
    ...
    uint8_t type_id() const override { return 2; }
};

ComplementColorSet::ComplementColorSet(const sdsl::bit_vector& bv) {
    uint32_t prev = 0;
    for (size_t c = 0; c < bv.size(); c++) {
        if (!bv[c]) {
            gaps.push_back((uint32_t)(c - prev));
...
size_t ComplementColorSet::size() const { return count; }
sdsl::bit_vector ComplementColorSet::to_bitvector(size_t n_colors) const {
    sdsl::bit_vector out(n_colors, 1);
    size_t pos = 0;
    for (auto g : gaps) {
        pos += g;
        out[pos] = 0;
...
void ComplementColorSet::serialize(std::ostream& out) const {
    uint8_t tid = type_id();
    out.write(reinterpret_cast<const char*>(&tid), sizeof(tid));
    out.write(reinterpret_cast<const char*>(&count), sizeof(count));
    size_t len = gaps.size();
...
void ComplementColorSet::load(std::istream& in, size_t /*n_colors*/) {
    in.read(reinterpret_cast<char*>(&count), sizeof(count));
    size_t len;
    in.read(reinterpret_cast<char*>(&len), sizeof(len));
    gaps.resize(len);
