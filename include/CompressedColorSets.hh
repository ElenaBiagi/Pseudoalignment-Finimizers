...
public:
    ...
    CompressedColorSets(const vector<pair<sdsl::bit_vector, vector<size_t>>> &deduplicated_cs, const uint64_t n_colors, vector<uint64_t> &color_set_ids)
    {
        ...
        for (auto &[bv, old_offsets] : deduplicated_cs)
        {
            ...
            else
            {
                ...
                for (size_t c = 0; c < n_colors; c++)
                {
                    if (!bv[c])
                    {
                        temp_cL.push_back(static_cast<uint16_t>(c));
                    }
                }
        ...
        for (size_t b = 0; b < color_set_ids.size(); b++)
        {
            ...
            if (cL_color_set_ids[b])
            {
                color_set_ids[b] += sparse_count;
            }
            // Add temp_EF_v.size()+cEF.size() (after the loop) to the indices of BV
            if (BV_color_set_ids[b])
            {
                color_set_ids[b] += dense_count;
            }
        }
    ...
    CompressedColorSets(const vector<pair<vector<size_t>, vector<size_t>>> &deduplicated_cs_sparse, const uint64_t n_colors, vector<uint64_t> &color_set_ids)
    {
        ...
        vector<uint64_t> temp_EF_v = {0};                                 // the first value has to be 0
        vector<uint64_t> temp_cEF_v = {};                                 // the first value is the last value of L
        sdsl::bit_vector temp_BV(deduplicated_cs_sparse.size() * n_colors, 0); // ensure that it's all 0s
        // BV.swap(temp_BV);
        size_t BV_size = 0;
        sdsl::bit_vector BV_color_set_ids(color_set_ids.size(), 0);
        sdsl::bit_vector cL_color_set_ids(color_set_ids.size(), 0);
        ...
        for (auto &[v_list, old_offsets] : deduplicated_cs_sparse)
        {
            const uint64_t start = old_offsets[0];

            // TODO access color_set_concat and save the value in a bv

            const size_t l_size = v_list.size();
            // Sparse
            if (l_size < sparse_thr)
            {
                // store 1s explicitly
                for (auto &c: v_list)
                {
                ...
                temp_EF_v.push_back(L.size()); // Keep track of ending pos // exclusive ends will be inclusive starts for the next interval
                L_sizes.push_back((float)l_size);
            }
            else if (l_size < dense_thr)
            {
                ...
                new_offset += n_colors;
                BV_sizes.push_back((float)l_size);
            }
            else
            {
                // very dense
                ...
                temp_cEF_v.push_back(temp_cL.size()); // Keep track of ending pos // exclusive ends will be inclusive starts for the next interval
                dense_sizes.push_back((float)(n_colors - l_size));
            }
        }
